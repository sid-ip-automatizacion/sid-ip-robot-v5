...
# back office management function
...
class UserEnvironment:
    ...
    def __init__(self):
        ...
        # Ruta absoluta al archivo icon.ico
        self.icon_path = Path(__file__).resolve(
        ).parent.parent / 'resources' / 'icon.ico'

        # Ruta absoluta al archivo azure.tcl
        self.theme_path = Path(__file__).resolve(
        ).parent.parent / 'resources' / 'azure.tcl'
        """
        ...
        self.__work_area = ttk.Frame(
            master=self.__my_canvas, padding=(10, 10))  # Area de trabajo
        ...
        self.__my_canvas.configure(
            yscrollcommand=self.scrollbar_vertical.set, xscrollcommand=scrollbar_horizontal.set)
        self.__my_canvas.grid(row=0, column=0, sticky="nsew")
        scrollbar_horizontal.grid(
            row=1, column=0, sticky="ew", ipadx=10, ipady=10)
        self.scrollbar_vertical.grid(
            row=0, column=1, sticky="ns", ipadx=10, ipady=10)
        self.__my_canvas.configure(
            scrollregion=self.__my_canvas.bbox(self.__windows_item))
        ...
        self.filemenu.add_command(
            label="Minimize", command=lambda: self.__root.iconify())
        self.filemenu.add_separator()
        self.filemenu.add_command(
            label="Close", command=lambda: self.__root.destroy())
        # Menu Mi cuenta
        self.myaccmenu = tkinter.Menu(self.menubar, tearoff=0)
        self.myaccmenu.add_command(
            label="Configure SCCD", command=self.env.set_sccd_credentials)
        self.myaccmenu.add_command(
            label="Change password", command=self.env.create_password)
        self.myaccmenu.add_command(
            label="Configure Meraki API Key", command=self.env.set_meraki_key)
        # Menu Funciones
    ...
    def run_states(self):
        ...
        sccd = SCCD(self.env.get_owner_sccd(),
                    self.env.get_user_sccd(), self.env.get_pass_sccd())
        ...
        if self.env.get_owner_sccd() == '' or self.env.get_user_sccd() == '' or self.env.get_pass_sccd() == '':
            ...
            # Habilita el boton si no se pudo abrir la ventana hija
            ...
            # Deshabilita el boton mientras se ejecuta la función
            ...
            run_sccd_manager(self.env.get_owner_sccd(),
                             ...
                             # Pasa el callback de cierre de la ventana hija
                             on_close=self._child_closed_sccd_m)
        else:
    ...
    def run_atp_sw(self):
        ...
        """
        self.clear_work_area()  # Limpia el area de trabajo
        # Ejecuta la venta  de ATP switches en el ambiente usuario
    ...
    def initial_work_area(self):
        ...
        self.clear_work_area()  # Limpia el area de trabajo
        self.__root.geometry("250x400")
        self.btn_sccd_m = ttk.Button(master=self.get_work_area(
        ), text="SCCD WO Management", command=self.run_states)
        self.btn_aps = ttk.Button(master=self.get_work_area(
        ), text="AP Management", command=self.run_aps)
        self.btn_sw_atp = ttk.Button(master=self.get_work_area(
        ), text="Meraki SW ATP", command=self.run_atp_sw)
        self.btn_sccd_maa = ttk.Button(master=self.get_work_area(
        ), text="SCCD Multi-Asset Assignment", command=self.run_multi_asset_assignment)
        self.btn_bo_m = ttk.Button(master=self.get_work_area(
        ), text="Back Office Management", command=self.run_bo_mgmt)
        ...
        if self.child_ref_sccd_m:
            # Deshabilita el boton si la ventana hija sigue abierta
            self.btn_sccd_m.config(state="disabled")
    ...
    def adjust_window(self, window):
        ...
        if window.winfo_width() < self.__root.winfo_width():
            self.__root.update_idletasks()
            self.__my_canvas.itemconfig(
                self.__windows_item, width=self.__root.winfo_width())
            self.__my_canvas.configure(
                scrollregion=self.__my_canvas.bbox(self.__windows_item))
        if window.winfo_height() < self.__root.winfo_height():
            self.__root.update_idletasks()
            self.__my_canvas.itemconfig(
                self.__windows_item, height=self.__root.winfo_height())
            self.__my_canvas.configure(
                scrollregion=self.__my_canvas.bbox(self.__windows_item))
    ...
    def _child_closed_sccd_m(self):
        # Habilita el boton cuando se cierra la ventana hija
        self.btn_sccd_m.config(state="normal")
        self.child_ref_sccd_m = False
    ...
    def show_about(self):
        ...
        about_text = tkinter.Label(about_win, text='version: 5.4.0'
                                   '\nSID-IP release'
